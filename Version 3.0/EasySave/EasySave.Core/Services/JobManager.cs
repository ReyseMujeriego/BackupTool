...
namespace EasySave.Core.Services
{
    ...
    public class JobManager
    {
        ...
        public event EventHandler<Job>? JobUpdated;
        ...
        public void RemoveJob(string jobName)
        {
            ...
            if (jobToRemove != null)
            {
                ...
                JobRemoved?.Invoke(this, jobToRemove);
            }
            else
            {
        ...
        public void UpdateJob(string jobName, string? newName, string? newSource, string? newDestination)
        {
            ...
            if (jobToUpdate != null)
            {
                OnMessageLogged?.Invoke(LanguageManager.GetString("OldJob", jobToUpdate.Name, jobToUpdate.SourcePath, jobToUpdate.DestinationPath));
                ...
                OnMessageLogged?.Invoke(LanguageManager.GetString("NewJob", jobToUpdate.Name, jobToUpdate.SourcePath, jobToUpdate.DestinationPath));
                ...
                JobUpdated?.Invoke(this, jobToUpdate);
            }
            else
            {
                OnMessageLogged?.Invoke(LanguageManager.GetString("JobNotFound", jobName));
        ...
        /// </returns>
        public List<Job> ListJobs(bool isConsoleMode)
        {
        ...
        public void ExecuteJob(List<string> extensions, int index)
        {
            if (index >= 0 && index < _jobs.Count)
            {
                var job = _jobs[index];
                OnMessageLogged?.Invoke(LanguageManager.GetString("ExecutingJob", job.Name));
                saver.PerformSaveSequential(_jobs);
            }
            else
            {
        ...
        {
            return Process.GetProcesses().Any(p => Instance.businessProcesses.Contains(p.ProcessName));
        }

        public void PauseJob()
        {
            saver.PauseJob(selectedJobs);
        }

        public void ResumeJob()
        {
            saver.ResumeJob(selectedJobs);
        }

        public void StopJob()
        {
            saver.StopJob(selectedJobs);
        }
    }
}