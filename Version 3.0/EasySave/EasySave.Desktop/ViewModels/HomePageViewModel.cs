...
namespace EasySave.Desktop.ViewModels
{
    ...
    {
        public static SaveManager SharedSaveManager { get; } = new SaveManager();
        ...
        private RemoteConsoleServer? server;
        ...
        public string ServerButtonText => IsServerRunning ? LanguageManager.GetString("StopServer") : LanguageManager.GetString("StartServer");
        ...
        {
            if (parameter is IList<object> selectedObjects)
            {
                SelectedJobs = new ObservableCollection<Job>(selectedObjects.Cast<Job>());
                foreach (var job in SelectedJobs)
                {
                    AppendConsoleLog($"- {job.Name} ({job.SourcePath} → {job.DestinationPath})");
                }
            }
        }
        ...
        private void OnMessageReceivedFromServer(object sender, string message)
        {
            AppendConsoleLog(message);
        }
        ...
        private void ExecuteMultipleJobs()
        {
            ...
            // Retrieve indices of selected jobs by matching names instead of object references
            List<int> indices = jobManager.ListJobs(false)
                .Select((j, i) => new { Job = j, Index = i })
                .Where(j => SelectedJobs.Any(selectedJob => selectedJob.Name == j.Job.Name)) // Compare by Name
                .Select(j => j.Index)
                .ToList();
            ...
            if (indices.Count > 0)
            {
                ...
                jobManager.ExecuteJobsSequentially(indices.ToArray());
        ...
        private void ConfigureEncryption()
        {
            ...
            if (enableEncryption)
            {
                ...
                string? extensionsInput = Microsoft.VisualBasic.Interaction.InputBox(LanguageManager.GetString("EnterEncryptionExtensions"), ".txt,.pdf");
        ...
        private void StartServer()
        {
            ...
            server = new RemoteConsoleServer(54285);

            server.Start();

            AppendConsoleLog(LanguageManager.GetString("ServerStarted"));
            IsServerRunning = true;
            server.MessageReceived += OnMessageReceivedFromServer;
            server.LogMessage += OnLogListened;
        }
        ...
        private void StopServer()
        {
            ...
            AppendConsoleLog(LanguageManager.GetString("ServerStopped"));
            server.Dispose();
